...
//
// This file adds the Gemini 3.5 Transcribe (speech-to-text) capability to the
// all-in-one mcp-gemini-go server. It uses the *synchronous* transcription API
// (generate_content on gemini-3.5-transcribe-preview), NOT the live/streaming
// API. The handler is a THIN wrapper around the shared common.Transcribe /
// common.RenderTranscribeResult helpers — the identical calls the standalone
// mcp-gemini-transcribe-go server makes — so the two servers can never drift.
...
// transcription plumbing: it parses/validates the tool arguments via
