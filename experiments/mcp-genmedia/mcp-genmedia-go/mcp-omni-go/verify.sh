...
response=$(mcptools tools ./mcp-omni-go)

# Check if the response contains the expected tool name
if [[ "$response" == *"omni_video_generation"* ]]; then
  echo "Verification successful: 'omni_video_generation' tool found."
else
  echo "Verification failed: Could not find 'omni_video_generation' tool in the server's response."
  echo "Response was:"
  echo "$response"
  exit 1
fi
