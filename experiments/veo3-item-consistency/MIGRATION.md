...
> [`experiments/veo3-character-consistency`](../veo3-character-consistency/MIGRATION.md),
> which is in the identical situation. Treat the two together — see the **Refactor
> note** below.
...
| `image_generator.py` | `generate_images_and_select_best()` | `EDIT_MODE_DEFAULT` + `SubjectReferenceImage` | Subject-customized generation of the item in the new scene |
...
| `extend_video/extend_image_generator.py` | `edit_model = config.IMAGEN_MODEL_NAME` | — | Image edit within the video-extension path |
...
1. **Replace the edit client.** Swap the `client.models.edit_image(...)` calls (in
   `image_generator.py`, `utils/outpainting.py`, and the `extend_video/` path) for
   Gemini prompt-based image editing that passes the reference image(s) plus a text
   prompt.
...
4. **Drop `IMAGEN_MODEL_NAME`** from `config.py` and point the code (including
   `extend_video/`) at the Gemini image model instead.
...
`experiments/veo3-item-consistency` and
[`experiments/veo3-character-consistency`](../veo3-character-consistency/MIGRATION.md)
are **near-duplicate codebases**: near-identical `config.py`, the same
...
`imagen-3.0-capability-001`). This item variant additionally carries an
